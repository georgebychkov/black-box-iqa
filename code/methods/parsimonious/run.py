...
def get_loss(model, compress_image, ref_image, q=80):
...
class LocalSearchHelper(object):
  ...
  def __init__(self, model, epsilon, max_iters, q):
  ...
  def perturb(self, image, ref_image, noise, blocks):
    ...
    loss = get_loss(self.model, image_batch, ref_image, self.q)
    #print(loss)
    num_queries += 1
    curr_loss = loss
    ...
    for _ in range(self.max_iters):
      ...
      for ibatch in range(num_batches):
        ...
        for i in range(bend-bstart):
          losses = get_loss(self.model, image_batch[i][None, :], ref_image, self.q)
          #print(losses)
      ...
      while len(priority_queue) > 0:
        ...
        losses = get_loss(self.model, image_batch, ref_image, self.q)
        #print(losses)
        num_queries += 1
        margin = -(losses-curr_loss)
...
def attack(compress_image, ref_image=None, model=None, metric_range=100, device='cpu', q=80):
    ...
    local_search = LocalSearchHelper(model, epsilon, max_iters, q)
