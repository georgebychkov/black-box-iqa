...
class MetricModel(torch.nn.Module):
    def __init__(self, device, model_path):
        ...
        model.eval()
        self.sample_args = {'fragments_h': 7, 'fragments_w': 7, 'fsize_h': 32, 'fsize_w': 32, 'aligned': 8, 'clip_len': 32, 'frame_interval': 2, 'num_clips': 1, 't_frag': 8}
    ...
    def forward(self, image, inference=False):
        ...
        if image.shape[1] == 4:
          image = torch.concatenate([image for _ in range(2)], axis=1)
        ...
        score = sigmoid_rescale(result.mean(), device=self.device, model='FasterVQA')
        return score
