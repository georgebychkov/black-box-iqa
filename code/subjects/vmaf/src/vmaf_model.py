...
class MetricModel(torch.nn.Module):
    ...
    def forward(self, ref, dist):
        ...
        vqmt_opt = "vqmt -quiet -metr vmaf over Y -set disable_clip=true -set model_preset=vmaf_v063" + \
            " -csv-file %s -orig %s -in %s  -threads 0" % ('res.csv', orig_path, dist_path)
        
        subprocess.run(shlex.split(vqmt_opt))
        
        res = float(pd.read_csv('res.csv')['Netflix VMAF_VMAF063_float_bootstrap'].iloc[-1])
